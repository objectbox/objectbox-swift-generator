...
enum ObjectBoxFilters {
    ...
    static func printError(_ error: Swift.Error) {
        if let obxError = error as? IdSync.Error {
            ...
            case .OutOfUIDs:
                print("error: Could not generate a unique UID in reasonable time.")
            case .SyncMayOnlyBeCalledOnce:
                print("error: sync() may only be called once.")
            ...
            case .CandidateUIDNotInPool(let uid):
                print("error: Candidate UID \(uid) was not in new UID pool.")
            ...
            case .DuplicateIdAnnotation(let entity, let found, let existing):
                print("error: Entity \(entity) has both \(found) and \(existing) annotated as 'objectId'. There can only be one.")
